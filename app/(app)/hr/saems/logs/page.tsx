import { getTermOptions, getLogs } from "@/app/(app)/hr/saems/logActions";
import { resolveSaemView } from "@/lib/saemAccess";
import LogsManager from "@/app/(app)/hr/saems/logs/LogsManager";

export const dynamic = "force-dynamic";

// SA-17: 확정취소·초기화가 saem 직무에도 열려 isM0 게이트가 필요 없어졌다.
// 2026-09: 조회는 로그인 직원 누구나. 확정·취소·초기화는 canManage 만.
export default async function LogsPage() {
  ...
  const [initial, view] = await Promise.all([
    getLogs(defaultTerm ? { termId: defaultTerm.id } : {}),
    resolveSaemView(),
  ]);
