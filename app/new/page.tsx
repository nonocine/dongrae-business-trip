        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {ACTIVITY_KINDS.map((kind) => (
            <Link
              key={kind}
              href={`/new/${kind}`}
              className={`flex flex-col items-center justify-center gap-2 rounded-2xl border-2 bg-card p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${ACTIVITY_CARD_CLASS[kind]}`}
            >
