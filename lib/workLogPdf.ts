...
//   글리프 유무 확인(hasGlyphForCodePoint)에 fontkit.create 가 필요해 default 로 받는다.
...
// 나눔고딕이 못 그리는 글자 대체.
//   강사가 ①②③ · ㎡ · ℃ 같은 글자를 쓰면 pdf-lib 는 말없이 빈칸을 찍는다.
//   빈칸은 "안 썼다"와 구분이 안 되는데 이 문서는 관장 결재까지 올라간다.
//   그래서 뜻이 통하는 글자로 바꾸고, 그래도 없으면 □ 로 눈에 띄게 남긴다.
let _regularFk: FontkitFont | null = null;
let _boldFk: FontkitFont | null = null;
function fkFont(isBold: boolean): FontkitFont {
  if (isBold) {
...
function fitToFont(s: string, fk: FontkitFont): string {
  let out = "";
  for (const ch of s) {
