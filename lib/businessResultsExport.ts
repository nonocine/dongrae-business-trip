...
export async function buildBusinessReportDocx(
  ...
  const children = [
    ...
    docTable(
      ...
      [450, 1450, 2700, 650, 850, 900, 850, 750, 900],
...
export async function buildBusinessReportWorkbook(
  ...
  input.results.forEach((r) =>
    results.addRow([
      ...
      r.youth_uses,
      r.other_uses,
      { formula: `F${results.rowCount + 1}+G${results.rowCount + 1}` },
      r.summary,
      r.evaluation,
  ...
  const totalRow = results.addRow([
    "합계",
    "",
    { formula: `SUM(C5:C${results.rowCount})` },
    { formula: `SUM(D5:D${results.rowCount})` },
    { formula: `SUM(E5:E${results.rowCount})` },
    { formula: `SUM(F5:F${results.rowCount})` },
    { formula: `SUM(G5:G${results.rowCount})` },
    { formula: `SUM(H5:H${results.rowCount})` },
    "",
    "",
    "",
    "",
  ...
  results.columns = [14, 24, 11, 11, 11, 12, 11, 14, 34, 34, 11, 12, 12].map(
    (width) => ({ width }),
  );
