...
## 낡은 자리표시자 회귀 방지 (placeholders.spec.ts)

`/hr` 의 계약서·증명서 탭이 "곧 제공될 예정입니다" 를 띄우고 있었습니다.
증명서는 이미 완성돼 **발급 18건**까지 되고 있었고 근로계약서도 인사기록카드
첨부서류로 보관 중이었는데, 그 한 줄 때문에 같은 기능을 세 번이나 "대기 중"
으로 착각했습니다. 문구가 아니라 **약속**을 검사합니다.
...
```bash
E2E_HR_M0=홍길동 npx playwright test e2e/placeholders.spec.ts
```

확인하는 것: 있는 기능 자리에 "곧 제공/예정입니다" 류 문구가 다시 나타나지
않는지, 각 안내가 **실제로 갈 곳(링크)** 을 들고 있는지(증명서는 관리자용
발급대장과 본인 신청 두 곳 모두), 그 링크가 실제로 열리는지. 아직 준비 중인
것(온나 실적)은 "준비 중" 을 지우라고 하지 않고 **대안 경로가 함께 있는지**만
봅니다 — 없는 기능을 있다고 적는 것도 같은 잘못이기 때문입니다.
